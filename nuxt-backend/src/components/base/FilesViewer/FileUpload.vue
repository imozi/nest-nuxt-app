  ...
  <UiDrawer v-model:open="isOpenDrawer">
    ...
    <UiDrawerContent>
      <div class="mx-auto w-full max-w-xl">
        <UiDrawerHeader class="flex items-center">
          <div class="flex flex-col">
            <UiDrawerTitle>{{ fileTypes[props.type].title }}</UiDrawerTitle>
            <UiDrawerDescription>{{ fileTypes[props.type].description }}</UiDrawerDescription>
          </div>
          <UiDrawerClose as-child>
            <UiButton variant="ghost" class="ml-auto w-auto"> Закрыть </UiButton>
          </UiDrawerClose>
        </UiDrawerHeader>

        <UiScrollArea class="min-h-48 p-5 pt-0">
          <div class="rounded-md">
            <div class="max-h-[30rem]">
              <FilePondComponent
                ...
                allow-multiple
                :accepted-file-types="fileTypes[props.type].types"
                allow-process="true"
...
.filepond--drop-label {
  @apply cursor-pointer text-muted-foreground transition-colors hover:text-zinc-800 hover:dark:text-white;
  position: absolute;
  left: 0;
  right: 0;
  top: 0;
...
.filepond--panel-root {
  @apply cursor-pointer rounded-lg border bg-card text-card-foreground;
}
